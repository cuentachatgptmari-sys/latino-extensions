...
class Pelisplushd : TmdbProvider() {
    ...
    override suspend fun load(url: String): LoadResponse? {
        ...
        val isAnime = genres?.contains("Animation") == true &&
                (res.original_language == "zh" || res.original_language == "ja")
            ...
            val episodes = res.seasons?.mapNotNull { season ->
                app.get("$tmdbAPI/${data.type}/${data.id}/season/${season.seasonNumber}?api_key=$apiKey&language=es-ES")
                    .parsedSafe<MediaDetailEpisodes>()?.episodes?.map { eps ->
                        newEpisode(
                            LoadData(res.title, year, isAnime, res.external_ids?.imdb_id,
                                eps.seasonNumber, eps.episodeNumber).toJson()
                        ) {
    ...
    override suspend fun loadLinks(
        ...
        val jsonString = res.selectFirst("script:containsData(dataLink)")?.data()
            ?.substringAfter("dataLink = ")?.substringBefore(";")
        ...
        if (jsonString != null) {
            ...
            for (i in 0 until jsonArray.length()) {
                ...
                if (decrypted?.success == true) {
                    ...
                    listForLang.addAll(decrypted.links.map { it.link })
                }
            }
        }
        ...
        val subUrl = if (season == null) "$subApiUrl/subtitles/movie/$id.json"
        else "$subApiUrl/subtitles/series/$id:$season:$episode.json"
        app.get(subUrl, timeout = 100L).parsedSafe<Subtitles>()?.subtitles?.amap {
            val lan = getLanguage(it.lang) ?: it.lang
            subtitleCallback(newSubtitleFile(lan, it.url))
        }
        return true
    }
}

data class Subtitles(val subtitles: List<Subtitle>, val cacheMaxAge: Long)
data class Subtitle(
    val id: String, val url: String,
    @param:JsonProperty("SubEncoding") val subEncoding: String,
    val lang: String, val m: String, val g: String
)
